...
public class TreeSetMethod_1 {
	public static void main(String[] args) {
		...
		// TreeSet 내의 메서드를 사용하기 위해 TreeSet으로 정의함. Set으로 정의해도 상관 없으나 TreeSet메서드를 사용할 수 없게 됨.
		TreeSet<Integer> treeSet = new TreeSet<>();
		for (int i=50; i>0; i-=2) {
			treeSet.add(i);
		}
		System.out.println(treeSet.toString());	//[2, 4, 6, 8, ... , 50]
		...
		for (int i = 0; i<treeSetSize; i++) {
			System.out.print(treeSet.pollFirst() + " ");	// 2, 4, ..., 50
		}
		System.out.println(" ");
		...
		System.out.println(treeSetSize);	//25
		for (int i = 0; i<treeSetSize; i++) {
			System.out.print(treeSet.pollLast() + " ");	// 50, 48, ..., 2
		}
		System.out.println(" ");
		System.out.println(treeSet.size());	//0
		
		//#9. SortedSet<E> headSet(E element)
		for (int i=50; i>0; i-=2) {				// 2, 4, ..., 50
			treeSet.add(i);
		}
		SortedSet<Integer> sSet = treeSet.headSet(20);
		System.out.println(sSet.toString());	// [2, 4, ..., 18]
		
		//#10. NavigableSet<E> headSet(E element, boolean inclusive)
		NavigableSet<Integer> nSet = treeSet.headSet(20, true);
		System.out.println(nSet.toString());	// [2, 4, ..., 20]
		
		nSet = treeSet.headSet(20, false);
		System.out.println(nSet.toString());	// [2, 4, ..., 18]
